...
export async function GET(req: NextRequest) {
  ...
  const slackRes = await fetch("https://slack.com/api/oauth.v2.access", { method: "POST", body: JSON.stringify({ code: slackTempCode, client_id: process.env.AUTH_SLACK_ID, client_secret: process.env.AUTH_SLACK_SECRET }) });
  ...
  if (!slackBody.access_token) return NextResponse.json({ error: "get_token_failed", message: "Failed to get the Slack authorization token, make sure the code is correct!" }, { status: 500 });
  await client.db(process.env.MONGODB_DB).collection<UserDoc>("statuses").updateOne({ user: session.user.email }, {
    $set: { slackToken: slackBody.access_token }
  });
