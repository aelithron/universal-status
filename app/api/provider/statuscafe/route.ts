...
export async function GET(req: NextRequest) {
  ...
  const statusCafeToken = req.nextUrl.searchParams.get("token");
  if (!statusCafeToken) return NextResponse.json({ error: "missing_token", message: "URL is missing a token parameter, please add a status.cafe CSRF token!" }, { status: 400 });
  if (!req.nextUrl.searchParams.get("cookie")) return NextResponse.json({ error: "missing_cookie", message: "URL is missing a cookie parameter, please add a base64-encoded version of the status.cafe cookie!" }, { status: 400 });
  const statusCafeCookie = atob(req.nextUrl.searchParams.get("cookie")!);
