...
- `AUTH_SLACK_ID` and `AUTH_SLACK_SECRET`: Optional, a Slack OAuth ID and Secret. Just like above, this enables Slack login, as well as the ability to set statuses on Slack. You can create a Slack app and add it's ID and secret here. No links are provided, as it's a complicated process to set up. Your redirect URLs are `https://[domain].[tld]/api/auth/callback/slack` and `https://[domain].[tld]/api/provider/slack` (add both to the box).
- `GITHUB_ID` and `GITHUB_SECRET`: Optional, a GitHub App ID and Secret. This enables setting statuses on GitHub, though not login. You can create a GitHub app and add it's ID and secret here. No links are provided, as it's a complicated process to set up. Your redirect URL is `https://[domain].[tld]/api/provider/github`.
